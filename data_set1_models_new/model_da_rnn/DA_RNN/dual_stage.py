...
import Generate_data as data_feeder
...
# Network Parameters
# encoder parameter
if data_feeder.data_set == 1:
    exog_dim = 81
if data_feeder.data_set == 2:
    exog_dim = 5

n_input_encoder = exog_dim  # n_feature of encoder input
...
encoder_input = tf.placeholder(
    "float",
    [None, n_steps_encoder, n_input_encoder]
)
decoder_input = tf.placeholder(
    "float",
    [None, n_steps_decoder, n_input_decoder]
)
decoder_gt = tf.placeholder(
    "float",
    [None, n_classes]
)
encoder_attention_states = tf.placeholder(
    "float",
    [None, n_input_encoder, n_steps_encoder]
)
...
with tf.Session() as sess:
    ...
    # read the input data
    Data = data_feeder.Input_data(batch_size, n_steps_encoder, n_steps_decoder, n_hidden_encoder)
    ...
    op_file = 'model_daRnn_dataset_' + str(data_feeder.data_set) + '_op.csv'
    f = open(op_file, 'w')
    f.write('Test Error')
